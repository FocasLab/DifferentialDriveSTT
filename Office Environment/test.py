...
#----------------------------------------------------------------------------#
#---------------------------------- TUBE 1 ----------------------------------#
solver1 = STT_Solver(4, 2, 0.25, [0.01, 0.01], [0.01, 0.01])

S_constraints_list = reach(solver1, 3.4, 3.6, 0.7, 0.9, 0, 1)
T1_constraints_list = reach(solver1, 3.9, 4.1, 5.5, 5.7, 4, 5)
O1_constraints_list = avoid(solver1, 2, 3.2, 0.8, 2, 0, 5)
O2_constraints_list = avoid(solver1, 3.2, 10, 3, 3.2, 0, 5)
O3_constraints_list = avoid(solver1, 3.6, 10, 4.6, 5, 0, 5)
...
for T in T1_constraints_list:
    solver1.solver.add(T)

for O in O1_constraints_list:
    solver1.solver.add(O)

for O in O2_constraints_list:
    solver1.solver.add(O)

for O in O3_constraints_list:
    solver1.solver.add(O)
...
#----------------------------------------------------------------------------#
#---------------------------------- TUBE 2 ----------------------------------#
solver2 = STT_Solver(6, 2, 0.25, [0.01, 0.01], [0.01, 0.01])

# S_constraints_list = reach(solver2, 3.9, 4.1, 5.5, 5.7, 0, 1)
# T2_constraints_list = reach(solver2, 8.4, 8.8, 3.6, 4.0, 9, 10)
T2_constraints_list = reach(solver2, 8.0, 8.4, 3.6, 4.0, 7, 8)
# T2_constraints_list = reach(solver2, 4.8, 5.2, 3.6, 4.0, 9, 10)
O2_constraints_list = avoid(solver2, 0.4, 4.6, 3.0, 3.2, 0, 10)
O3_constraints_list = avoid(solver2, 5.0, 5.4, 4.6, 7.0, 0, 10)
# O4_constraints_list = avoid(solver2, 0.0, 8.4, 0, 3.2, 0, 10)
...
for S in S_constraints_list:
    solver2.solver.add(S)

for T in T2_constraints_list:
    solver2.solver.add(T)
...
#----------------------------------------------------------------------------#
#---------------------------------- TUBE 3 ----------------------------------#
solver3 = STT_Solver(4, 2, 0.25, [0.01, 0.01], [0.01, 0.01])

T3_constraints_list = reach(solver3, 9.8, 10.2, 0.4, 0.8, 9.75, 10.5)
O2_constraints_list = avoid(solver3, 7.2, 8.2, 0.0, 3.2, 0, 10.5)
O3_constraints_list = avoid(solver3, 9.2, 10.4, 1.6, 8.4, 0, 10.5)
O4_constraints_list = avoid(solver3, 8.8, 11.4, 1.0, 1.2, 0, 10.5)

# for S in S_constraints_list:
#     solver3.solver.add(S)

for T in T3_constraints_list:
    solver3.solver.add(T)
...
solver3.join_constraint(tube2, solver2, 7)
