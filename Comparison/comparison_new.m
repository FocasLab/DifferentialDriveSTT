...
for d_case = 1:length(use_disturbance)
    ...
    %     safe = true;
    %     for j = 1:M
    %         if norm(xd(1:2) - obs(j,1:2)') <= Reff(j)
    %             safe = false;
    %             break;
    %         end
    %     end
    %     if norm([xd(end-1), yd(end-1)] - goal) <= goal_pos_tol && safe
    %         success_count = success_count + 1;
    %     end
    % end
    ...
    %         u = z_sol(1:2);
    % 
    %         x = x + dt_sim * ([u(1)*cos(x(3)); u(1)*sin(x(3)); u(2)] + dist);
    ...
    %     safe = true;
    %     % for j = 1:M
    %     %     if norm(x(1:2) - obs(j,1:2)') <= Reff(j)
    %     %         safe = false;
    %     %         break;
    %     %     end
    %     % end
    %     % if norm(goal - x(1:2)) <= goal_pos_tol && safe
    %     %     success_count = success_count + 1;
    %     % end
    % end
    % fprintf('MPC Time: %.4f ± %.4f ms | Success Rate: %.2f%%', mean(time_array), std(time_array), 100*success_count/N_runs);
end
